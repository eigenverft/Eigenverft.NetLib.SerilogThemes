...
The demo keeps local artifacts intentionally easy to find: `bin/Debug`, `bin/Release`, and `bin/Publish`. A normal Release publish creates a self-contained Windows x64 single-file executable in `bin/Publish`:

```shell
dotnet publish src/prj/Eigenverft.NetLib.SerilogThemes.Console/Eigenverft.NetLib.SerilogThemes.Console.csproj -c Release
```
